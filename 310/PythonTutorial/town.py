...
class Town:
    ...
    def getLowestFruitCostPerPoundOnRoute(self, fruit, routeShops):
        """
            fruit: Fruit string
    ...
    def allFruitsCarriedAtShops(self, orderList, shops):
        ...
        Returns whether all fruit in the order list can be purchased at least
        one of these shops.
        """
        return None not in [self.getLowestFruitCostPerPoundOnRoute(fruit, shops)
    ...
    def getDistance(self, loc1, loc2):
        ...
        print("Error! getDistance(): loc1= {} or loc2 = {} is not in distance list!", loc1, loc2)
        return 0
    ...
    def getTotalDistanceOnRoute(self, route):
        ...
        if not route:
            return 0
        totalDistance = self.getDistance('home', route[0].getName())
        for i in range(len(route) - 1):
            totalDistance += self.getDistance(route[i].getName(), route[i + 1].getName())
        totalDistance += self.getDistance('home', route[-1].getName())
    ...
    def getPriceOfOrderOnRoute(self, orderList, route, gasCost):
        ...
        for fruit, numPounds in orderList:
            costPerPound = self.getLowestFruitCostPerPoundOnRoute(fruit, route)
